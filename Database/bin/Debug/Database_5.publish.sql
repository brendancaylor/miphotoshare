/*
Deployment script for MtPhotos

This code was generated by a tool.
Changes to this file may cause incorrect behavior and will be lost if
the code is regenerated.
*/

GO
SET ANSI_NULLS, ANSI_PADDING, ANSI_WARNINGS, ARITHABORT, CONCAT_NULL_YIELDS_NULL, QUOTED_IDENTIFIER ON;

SET NUMERIC_ROUNDABORT OFF;


GO
:setvar DatabaseName "MtPhotos"
:setvar DefaultFilePrefix "MtPhotos"
:setvar DefaultDataPath "C:\Program Files\Microsoft SQL Server\MSSQL11.SQL2012\MSSQL\DATA\"
:setvar DefaultLogPath "C:\Program Files\Microsoft SQL Server\MSSQL11.SQL2012\MSSQL\DATA\"

GO
:on error exit
GO
/*
Detect SQLCMD mode and disable script execution if SQLCMD mode is not supported.
To re-enable the script after enabling SQLCMD mode, execute the following:
SET NOEXEC OFF; 
*/
:setvar __IsSqlCmdEnabled "True"
GO
IF N'$(__IsSqlCmdEnabled)' NOT LIKE N'True'
    BEGIN
        PRINT N'SQLCMD mode must be enabled to successfully execute this script.';
        SET NOEXEC ON;
    END


GO
USE [$(DatabaseName)];


GO
PRINT N'Creating [dbo].[MtDbPhoto]...';


GO
CREATE TABLE [dbo].[MtDbPhoto] (
    [Id]           UNIQUEIDENTIFIER NOT NULL,
    [MtDbFolderId] UNIQUEIDENTIFIER NOT NULL,
    [DbName]       NVARCHAR (255)   NOT NULL,
    [DbPath]       NVARCHAR (255)   NOT NULL,
    [DbShareUrl]   NVARCHAR (255)   NOT NULL,
    [ViewingCode]  NVARCHAR (20)    NOT NULL,
    [TotalSold]    INT              NOT NULL,
    [TotalSales]   DECIMAL (18, 2)  NOT NULL,
    [LargeImage]   IMAGE            NOT NULL,
    [SmallImage]   IMAGE            NOT NULL,
    CONSTRAINT [PK_MtDbPhoto] PRIMARY KEY CLUSTERED ([Id] ASC)
);


GO
PRINT N'Creating [dbo].[MtDbPhotoSale]...';


GO
CREATE TABLE [dbo].[MtDbPhotoSale] (
    [Id]          UNIQUEIDENTIFIER NOT NULL,
    [MtDbPhotoId] UNIQUEIDENTIFIER NOT NULL,
    [SaleCode]    NVARCHAR (20)    NOT NULL,
    [PricePaid]   DECIMAL (18, 2)  NOT NULL,
    [DatePaid]    DATETIME         NOT NULL,
    [IpnMessage]  NVARCHAR (MAX)   NOT NULL,
    CONSTRAINT [PK_MtDbPhotoSale] PRIMARY KEY CLUSTERED ([Id] ASC)
);


GO
PRINT N'Creating [dbo].[FK_MtDbPhoto_MtDbFolder]...';


GO
ALTER TABLE [dbo].[MtDbPhoto] WITH NOCHECK
    ADD CONSTRAINT [FK_MtDbPhoto_MtDbFolder] FOREIGN KEY ([MtDbFolderId]) REFERENCES [dbo].[MtDbFolder] ([Id]);


GO
PRINT N'Creating [dbo].[FK_MtDbPhotoSale_MtDbPhoto]...';


GO
ALTER TABLE [dbo].[MtDbPhotoSale] WITH NOCHECK
    ADD CONSTRAINT [FK_MtDbPhotoSale_MtDbPhoto] FOREIGN KEY ([MtDbPhotoId]) REFERENCES [dbo].[MtDbPhoto] ([Id]);


GO
PRINT N'Checking existing data against newly created constraints';


GO
USE [$(DatabaseName)];


GO
ALTER TABLE [dbo].[MtDbPhoto] WITH CHECK CHECK CONSTRAINT [FK_MtDbPhoto_MtDbFolder];

ALTER TABLE [dbo].[MtDbPhotoSale] WITH CHECK CHECK CONSTRAINT [FK_MtDbPhotoSale_MtDbPhoto];


GO
PRINT N'Update complete.';


GO
